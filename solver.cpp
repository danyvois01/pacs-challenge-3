...
#pragma omp parallel for reduction(max:local_error) // Parallel loop over local grid points with reduction operation for error
        ...
        if (size > 1) { // If there are more than one processes
            ...
            if (rank < size - 1) { // If the current process is not the last one
                std::copy(local_u.end() - 2 * n, local_u.end() - n, send_buffer.begin()); // Copy boundary points to the send buffer
                ...
                std::copy(recv_buffer.begin(), recv_buffer.end(), local_u.end() - n); // Copy received boundary points to the local solution vector
            }

            if (rank > 0) { // If the current process is not the first one
                std::copy(local_u.begin() + n, local_u.begin() + 2 * n, send_buffer.begin()); // Copy boundary points to the send buffer
                MPI_Sendrecv(send_buffer.data(), n, MPI_DOUBLE, rank - 1, 1, // Send and receive boundary points
                            ...
                            MPI_COMM_WORLD, MPI_STATUS_IGNORE);
                std::copy(recv_buffer.begin(), recv_buffer.end(), local_u.begin()); // Copy received boundary points to the local solution vector
            }
        }
    }
    ...
    if (rank == 0) { // If the current process is the root process
        std::cout << "Number of iterations (parallel): " << iter << std::endl; // Print the number of iterations
        std::cout << "L2 norm of the error (parallel): " << calculate_L2_norm(data) << std::endl; // Print the L2 norm
        std::cout << "Total execution time (parallel): " << elapsed.count() << " seconds" << std::endl; // Print the execution time
